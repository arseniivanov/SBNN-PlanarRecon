...
class RecurrentSNN_v3(nn.Module):
    ...
    # Adding the horizontal cell-like layer
            out_horizontal = self.horizontal_cell(x[step])
            out_horizontal = self.bn_horizontal(out_horizontal)
            spike_horizontal = self.leaky_horizontal(out_horizontal)
            spike_horizontal = k_wta(spike_horizontal, sparsity_level=0.6)

            # Subtract horizontal cell activity from spike1
            combined_spike1 = spike1 - spike_horizontal 


            if recurrent_spike is not None:
                combined_spike1 = combined_spike1 + recurrent_spike  # Add recurrent connection
